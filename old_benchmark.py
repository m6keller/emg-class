...
"""
Much of this code was taken from example code from biolab-put found here: https://github.com/biolab-put/putemg_examples/blob/48554520b69584cad48530e9339cec37a457555b/shallow_learn.py
"""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        putemg_folder = os.path.abspath("./features/subj_3/data")
        result_folder = os.path.abspath("./features/subj_3/")
        print("Using hardcoded test data and result folders")
    ...
    if '-nc' not in sys.argv:
        ...
        for file in all_files:
            ...
            ft: pd.DataFrame = putemg_features.features_from_xml('./benchmark_features.xml',
            ...
            #           format='table', 
            #           mode='w', complevel=5)
            ft.to_csv(os.path.join(calculated_features_folder, output_file),
                      index=False
                    #   'data', 
                    #   format='table', 
                    #   mode='w', complevel=5
    ...
    for r in records_filtered_by_subject:
        ...
        dfs[r] = pd.DataFrame(pd.read_csv(os.path.join(calculated_features_folder,
